...
def socket_security_level(security_level):
    if security_level == 'low':
...
def int_to_bytes(input):
    output_byte = b''
    for num in input:
        temp_byte = num.to_bytes(4,byteorder='big')
        output_byte += temp_byte
    return output_byte

# int 형식에서 bytes 형식으로 변경하는 함수
def int_to_bytes2(input):
    output_byte = b''
    for num in input:
...
def bytes_to_int(input_bytes):
    ...
    for i in range(0, len(input_bytes), 4):
        int_value = int.from_bytes(input_bytes[i:i+4], byteorder='big')
...
if __name__ == "__main__":
    ...
    ct2 = CHAM_CTR_Encryption(pt2, rk, len(pt2), k, w, r)
    dt2 = CHAM_CTR_Encryption(ct2, rk, len(pt2), k, w, r)
